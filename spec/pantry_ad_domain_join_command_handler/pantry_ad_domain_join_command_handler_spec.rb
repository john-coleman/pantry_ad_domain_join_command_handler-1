...
describe Wonga::Daemon::PantryAdDomainJoinCommandHandler do
  ...
  subject { described_class.new("domain", "username", "password", publisher, double.as_null_object) }
  ...
  describe "#winrm_get_domain_state" do
    ...
    it "determines a machine is on a domain" do
      win_rm_runner.stub(:run_commands).and_yield('test', "has been verified")
      expect(subject.winrm_get_domain_state(message, win_rm_runner)).to be_true
    end

    it "determines a machine is not on a domain" do
      win_rm_runner.stub(:run_commands).and_yield('test', "has not been verified")
      expect(subject.winrm_get_domain_state(message, win_rm_runner)).to be_false
    end
  end
  ...
  describe "#winrm_join_domain" do
    ...
    before(:each) do
      win_rm_runner.stub(:run_commands).and_yield('test', join_domain_data)
    end
  ...
  describe "#winrm_get_hostname" do
    ...
    before(:each) do
      win_rm_runner.stub(:run_commands).and_yield('test', get_hostname_data )
    end

    it "runs commands via WinRM" do
      subject.winrm_get_hostname(message, win_rm_runner)
      expect(win_rm_runner).to have_received(:run_commands)
    end

    it "receives a hostname via WinRM" do
      expect(subject.winrm_get_hostname(message, win_rm_runner)).to eq (get_hostname_data)
    end
  end
  ...
  describe "#winrm_set_hostname" do
    ...
    before(:each) do
      win_rm_runner.stub(:run_commands).and_yield('test', set_hostname_data)
    end
